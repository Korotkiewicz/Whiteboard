...
pl.edu.libratus.orbiter = {
    ...
    room: {
        id: "libratus.school.hallway"
    },
    server: {
        IP: '178.216.200.135',//"tryunion.com"
        PORT: 9100
    },
    whiteboard: {
        canvas: {
...
function init (l, p, c) {
    ...
    if(c) {
        pl.edu.libratus.orbiter.whiteboard.tool.cursor = c;
    }
...
function initCanvas () {
    ...
    // Size canvas
    canvas.width  = getComputedWidth('whiteboard');
    canvas.height = 404;
...
function readyListener (e) {
    ...
    chatRoom = orbiter.getRoomManager().createRoom(pl.edu.libratus.orbiter.room.id);
...
function touchDownListener (e) {
    ...
    var touchX = e.changedTouches[0].clientX - canvas.offsetLeft - canvasWrapper.offsetLeft;
    var touchY = e.changedTouches[0].clientY - canvas.offsetTop - canvasWrapper.offsetTop;
...
function pointerDownListener (e) {
    ...
    var mouseX = event.clientX - canvas.offsetLeft - canvasWrapper.offsetLeft;
    var mouseY = event.clientY - canvas.offsetTop - canvasWrapper.offsetTop;
...
function toolSelectListener (e) {
    ...
    if(pl.edu.libratus.orbiter.whiteboard.tool.cursor[localTool]) {
        canvas.style.cursor = pl.edu.libratus.orbiter.whiteboard.tool.cursor[localTool];
    }
